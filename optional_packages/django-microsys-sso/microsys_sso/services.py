...
def provider_setting(name, default=None):
    config = getattr(settings, "MICROSYS_SSO_PROVIDER", {})
...
def get_policy_for_application(application):
    try:
        return application.microsys_sso_policy
...
def build_userinfo_claims(user, application):
    ...
    claims = {
        ...
        "microsys_sso_client_id": getattr(application, "client_id", ""),
        "microsys_sso_role": decision.role,
        "microsys_sso": {
