...
def _render_detail_value(kind, label, value):
    return format_html(
        '<div class="ms-log-detail-value ms-log-detail-value--{}">'
        '<div class="ms-log-detail-value-label">{}</div>'
        '<div class="ms-log-detail-value-body">{}</div>'
        '</div>',
        kind,
        label,
        escape(value),
...
def render_log_details_panel(details):
    ...
    if 'filename' in details:
        ...
        items.append(format_html(
            '<div class="ms-log-detail-item">'
            '<div class="ms-log-detail-head">'
            '<div class="ms-log-detail-field">{}</div>'
            '<span class="ms-log-detail-status is-info"><i class="bi bi-info-circle"></i>{}</span>'
            '</div>'
            '<div class="ms-log-detail-values">{}</div>'
            '</div>',
        ...
        for field, changes in details.items():
            ...
            items.append(format_html(
                ...
                '<div class="ms-log-detail-head">'
                '<div class="ms-log-detail-field">{}</div>'
                '<span class="ms-log-detail-status {}"><i class="bi {}"></i>{}</span>'
                '</div>'
                '<div class="ms-log-detail-values">{}</div>'
                '</div>',
                field_label,
    ...
    return mark_safe('<div class="ms-log-details">{}</div>'.format("".join(str(item) for item in items)))
