...
from .utils import build_config_groups, get_client_ip, get_email_service_status, get_system_config, send_microsys_mail
...
def registration_throttle_allows(request, email):
    ...
    ip_count = _increment_throttle(f'microsys:registration:ip:{ip}', REGISTRATION_THROTTLE_SECONDS)
    email_count = _increment_throttle(f'microsys:registration:email:{email_key}', REGISTRATION_THROTTLE_SECONDS)
...
def create_inactive_registration_user(form, request):
    ...
    user._microsys_public_registration = True
    user.save()
    registration, token = PublicRegistration.create_for_user(
        ...
        ttl_seconds=getattr(settings, 'MICROSYS_REGISTRATION_TOKEN_TTL_SECONDS', REGISTRATION_TOKEN_TTL_SECONDS),
    )
...
def send_registration_verification_email(request, registration, token):
    ...
    context = {
        ...
        'system_name': identity.get('display_name') or 'microSYS',
    }
