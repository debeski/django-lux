"""Full system backup & restore (.msb files).

This is deliberately separate from the reports backup (``microsys.reports``):
...
Container format (``.msb`` — "microsys backup"):

    MSB1 | u32 metadata length | metadata JSON (cleartext, non-sensitive)
...
_SYSTEM_BACKUP_EXCLUDED = {
    ...
    "auth.permission",
    "admin.logentry",
    "microsys.reportbackup",
    "microsys.systembackup",
    "microsys.systemrestore",
}

_SUPERUSER_PASSWORD_OMITTED = "!microsys-superuser-password-omitted"


def _backup_config():
    config = getattr(settings, "MICROSYS_CONFIG", {}).get("backup", {})
...
def _dependency_sorted(models_list):
    ...
    projects often list microsys before django.contrib.auth), so restores must
...
def _microsys_version():
    try:
        import microsys
        return str(getattr(microsys, "__version__", "")) or "unknown"
    except Exception:
        return "unknown"
...
def _django_secret_key_seed():
    return str(getattr(settings, "SECRET_KEY", "") or "microsys-backup-secret-dev-key")
...
def _derive_backup_key(salt_hex, *, encryption=None, passphrase=None):
    ...
    # Backward-compatible reader for early unreleased MSB1 files. Only Django
...
def write_msb_container(zip_fileobj, dest, metadata, *, passphrase=None):
    """Wrap an already-built backup zip stream into an encrypted .msb container."""
    ...
    metadata.setdefault("format", MSB_FORMAT_VERSION)
    metadata.setdefault("kind", "microsys-system-backup")
    metadata["encryption"] = encryption
    payload = json.dumps(metadata, ensure_ascii=False).encode("utf-8")
    dest.write(MSB_MAGIC)
...
def read_msb_metadata(fileobj):
    """Read and return the cleartext metadata header; leaves the stream at the payload."""
    magic = fileobj.read(len(MSB_MAGIC))
    if magic != MSB_MAGIC:
        raise ValueError("Not a Microsys backup (.msb) file")
    ...
    if metadata.get("kind") != "microsys-system-backup":
        raise ValueError("Unsupported backup kind")
    return metadata


def decrypt_msb_to_tempfile(fileobj, *, passphrase=None):
    """Decrypt an .msb stream (positioned anywhere) into a temp zip file.

    Returns ``(metadata, tempfile)`` with the temp file positioned at 0.
    The caller owns closing the temp file.
    """
    fileobj.seek(0)
    metadata = read_msb_metadata(fileobj)
...
def write_system_backup(dest, *, passphrase=None):
    ...
    manifest = {
        "kind": "microsys-system-backup",
        "generated_at": timezone.now().isoformat(),
        "microsys_version": _microsys_version(),
    ...
    with tempfile.TemporaryFile() as zip_tmp:
        ...
        zip_tmp.seek(0)
        metadata = write_msb_container(zip_tmp, dest, {
            "created_at": manifest["generated_at"],
            "microsys_version": manifest["microsys_version"],
...
def run_system_backup(backup_pk, passphrase=None):
    """Celery-or-inline runner that builds the .msb for a SystemBackup row."""
    ...
    try:
        with tempfile.TemporaryFile() as tmp:
            ...
            saved_path = default_storage.save(
                f"{get_backup_storage_prefix()}/system-{backup.token}.msb",
                File(tmp),
            )
...
def _log_system_action(username, action, details):
    try:
        ...
        UserActivityLog.safe_log(
            user=user,
            action=action,
            model_name="Microsys System Backup",
            details=details,
        )
    except Exception:
        pass
...
def build_migration_report(manifest):
    ...
    return {
        ...
        "backup_microsys_version": manifest.get("microsys_version"),
        "current_microsys_version": _microsys_version(),
    }
...
def _wipe_and_load(zf, models_to_restore):
    ...
    """
    from .signals import suspend_microsys_signals

    counts = {}
    current_superuser_passwords = _current_superuser_passwords()
    with suspend_microsys_signals():
...
def run_system_restore(restore_pk, passphrase=None):
    """Celery-or-inline runner that fully replaces system data from an .msb file."""
    ...
    try:
        ...
        with default_storage.open(restore.backup_file_path, "rb") as fh:
            metadata, zip_tmp = decrypt_msb_to_tempfile(fh, passphrase=passphrase)
        try:
