(function () {
    ...
    const OPTIONS_ORDER_STORAGE_KEY = 'microsys.options.card-order.v1';
    ...
    function initDensityPickers() {
        ...
        const densityOptions = densityPicker
            ? Array.from(densityPicker.querySelectorAll('.ms-density-option[data-table-density]'))
            : [];
        const sidebarDensityPicker = document.querySelector('.ms-sidebar-density-picker');
        const sidebarDensityOptions = sidebarDensityPicker
            ? Array.from(sidebarDensityPicker.querySelectorAll('.ms-density-option[data-sidebar-density]'))
            : [];
    ...
    function initResetDefaults(grid) {
        const panel = document.querySelector('.ms-options-panel--danger[data-reset-url]');
    ...
    function persistCardOrder(grid, storageKey) {
        const order = Array.from(grid.querySelectorAll('.ms-options-card[data-options-card]'))
    ...
    function restoreCardOrder(grid, storageKey) {
        ...
        const cardsByKey = new Map(
            Array.from(grid.querySelectorAll('.ms-options-card[data-options-card]'))
    ...
    function initCardOrdering(grid) {
        const cards = Array.from(grid.querySelectorAll('.ms-options-card[data-options-card]'));
        if (cards.length < 2) {
            return;
        }
        ...
        grid.querySelectorAll('[data-options-card-handle]').forEach((handle) => {
            const card = handle.closest('.ms-options-card[data-options-card]');
            if (!card) {
                return;
            }
        ...
        grid.querySelectorAll('.ms-options-card[data-options-card]').forEach((card) => {
