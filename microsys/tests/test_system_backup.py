...
from microsys.backup import (
    decrypt_msb_to_tempfile,
    read_msb_metadata,
    run_system_backup,
    run_system_restore,
    write_system_backup,
...
class MsbContainerTests(TestCase):
    def test_container_round_trip(self):
        admin = User.objects.create_superuser('msb-admin', 'a@example.com', 'pass12345')
        original_hash = admin.password
        buffer = io.BytesIO()
        metadata, manifest = write_system_backup(buffer)
        self.assertEqual(metadata['kind'], 'microsys-system-backup')
        self.assertGreater(metadata['rows'], 0)

        buffer.seek(0)
        header_meta = read_msb_metadata(buffer)
        self.assertEqual(header_meta['rows'], metadata['rows'])
        self.assertTrue(header_meta['encryption']['salt'])

        read_meta, zip_tmp = decrypt_msb_to_tempfile(buffer)
        try:
            with zipfile.ZipFile(zip_tmp) as zf:
                inner = json.loads(zf.read('manifest.json'))
                self.assertEqual(inner['kind'], 'microsys-system-backup')
                ...
                self.assertNotIn('microsys.systembackup', {key.lower() for key in model_keys})
                users = json.loads(zf.read('data/auth/user.json'))
                admin_payload = next(item for item in users if item['fields']['username'] == 'msb-admin')
    ...
    def test_passphrase_protected_container_requires_passphrase(self):
        User.objects.create_superuser('msb-admin', 'a@example.com', 'pass12345')
        buffer = io.BytesIO()
        ...
        with self.assertRaises(ValueError):
            decrypt_msb_to_tempfile(buffer)

        buffer.seek(0)
        with self.assertRaises(Exception):
            decrypt_msb_to_tempfile(buffer, passphrase='wrong-pass')

        buffer.seek(0)
        read_meta, zip_tmp = decrypt_msb_to_tempfile(buffer, passphrase='vault-pass')
        try:
    ...
    def test_rejects_non_msb_file(self):
        with self.assertRaises(ValueError):
            read_msb_metadata(io.BytesIO(b'PK\x03\x04 definitely a zip'))
    ...
    def test_models_are_dependency_ordered(self):
        ...
        deferred user_id on hosts that list microsys before django.contrib.auth)."""
        from microsys.backup import get_system_backup_models

        ordered = get_system_backup_models()
        index = {model._meta.label_lower: i for i, model in enumerate(ordered)}
        self.assertLess(index['auth.user'], index['microsys.profile'])
        self.assertLess(index['auth.group'], index['auth.user'])
        self.assertLess(index['microsys.scope'], index['microsys.profile'])
    ...
    def test_full_backup_and_restore_replaces_everything(self):
        with tempfile.TemporaryDirectory() as media_root:
            with override_settings(MEDIA_ROOT=media_root):
                ...
                # preserved because superuser hashes are omitted from .msb files.
    ...
    def test_restore_blocks_on_migration_mismatch(self):
        with tempfile.TemporaryDirectory() as media_root:
            with override_settings(MEDIA_ROOT=media_root):
                ...
                with mock.patch(
                    'microsys.backup.get_current_migration_state',
