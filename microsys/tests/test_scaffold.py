...
from microsys.cli import main
from microsys.scaffold import _register_app
...
class ScaffoldTests(unittest.TestCase):
    def test_startproject_creates_expected_files(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            self.assertIn(f"Generated with django-microsys {__version__}.", manage_contents)
            ...
            self.assertIn("from microsys.utils import get_secret, microsys_settings", settings_contents)
            self.assertIn('SECRET_KEY = get_secret("DJANGO_SECRET_KEY", "DJANGO_SECRET_KEY")', settings_contents)
            self.assertIn("microsys_settings(globals())", settings_contents)
            ...
            self.assertIn('CORS_ALLOW_ALL_ORIGINS = True', settings_contents)
            self.assertIn('CORS_ALLOWED_ORIGINS = [BASE_URL]', settings_contents)
            ...
            self.assertIn('path("", include("microsys.urls"))', urls_contents)
            ...
            self.assertEqual(len([line for line in env_contents.splitlines() if line.strip()]), 6)
    ...
    def test_register_updates_settings_and_urls_idempotently(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            self.assertIn("# MicroSys generated apps start", settings_contents)
