...
DEVICE_ID_COOKIE_NAME = 'microsys_device_id'
DEVICE_ID_COOKIE_SALT = 'microsys.device_identity'
...
def remember_request_presence(request, *, trusted_device=None, force=False):
    ...
    if not isinstance(existing, dict):
        existing = {}
    raw_token = getattr(request, '_microsys_device_identity_token', '')
    cookie_needs_set = bool(getattr(request, '_microsys_device_identity_cookie_needs_set', False))
    if not raw_token:
        raw_token, cookie_needs_set = get_or_create_device_token(request)
        request._microsys_device_identity_token = raw_token
        request._microsys_device_identity_cookie_needs_set = cookie_needs_set
...
def attach_presence_cookie(response, request):
    raw_token = getattr(request, '_microsys_device_identity_token', '')
    needs_set = bool(getattr(request, '_microsys_device_identity_cookie_needs_set', False))
    if needs_set and raw_token:
        set_device_identity_cookie(response, raw_token, request=request)
        request._microsys_device_identity_cookie_needs_set = False
    return response
