...
- `dlux/release-manifest.json` is the version source: unreleased inline-safe v1.2.11 (test-only: anchors test `MEDIA_ROOT` to a temp dir, no migrations); latest published tag is v1.2.10 (migration `0006` adds `SystemBackup.media_included` + `DluxUpdateRun.backup_mode`).
