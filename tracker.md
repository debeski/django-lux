...
- **v1.9.3 (in development)**: the deployment and the resident Composer are two rows in the Application updates card, behaving like the DjangoLux/image rows (check icon, update icon only when there is one, tick otherwise), findings in a modal; one deployment check that offers the repair it found; read-only `agent-check` so the Composer update asks the registry before asking for a password; operations gated on the resident Composer (needs 1.5.2); interval slider; eye toggle on every password field; login says why a session cookie cannot be stored. Needs Composer 1.5.2. Not released.
