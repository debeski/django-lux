...
## What "required" means (and does not)

Composer is required **for inline updates**, not for DjangoLux to run. Inline
updates are opt-in and off by default: `DLUX_INLINE_UPDATES_ENABLED` defaults to
`False`, and the generated Compose scaffold is what turns it on. So:

- **A local checkout, a bare `runserver`, a deployment without Compose** — works
  fully. The updater never engages; reads of an absent runtime volume return
  empty, and queueing an update refuses with a message.
- **A Compose deployment without Composer** — works fully, minus updates. The
  panel shows the check as *unknown* rather than failing, and says why.
- **Enabling inline updates without Composer** — the only unsupported
  combination. There is nothing to execute the request.

The same shape applies to the other optional services. `smtp-relay` is one
possible value of `EMAIL_HOST`; DjangoLux never hardcodes it, so any SMTP host
or a non-SMTP `EMAIL_BACKEND` works, and a deployment without it simply sends no
mail (`dlux doctor` reports this rather than blocking). Enabling inline updates
without a writable runtime volume raises a named configuration error naming
`DLUX_UPDATE_RUNTIME_ROOT`, not an `OSError` from inside `mkdir`.

## Boot order (1.8.0)

Generated projects require **Docker Compose 5.3.0+**. The runtime reconcile and
the migrations run as Compose *init containers* — `pre_start` steps on the
`celery` service — each in its own ephemeral container that must exit 0 before
the service starts. A failed migration therefore stops the stack rather than
producing a half-started one, and the ordering holds on a plain
`docker compose up` with Composer absent.

Two details that are not obvious and are load-bearing:

- **The steps are on `celery`, not `web`.** A `pre_start` step inherits its
  service's volume mounts and cannot override them. `dlux_reconcile` writes the
  runtime pointer, and `web` mounts `dlux_runtime` read-only — there it would
  swallow the error (it is written never to block boot) and silently no-op
  forever.
- **Each step sets `DLUX_BOOT_GATE=off`.** A step also inherits the service's
  entrypoint, which waits for migrations to be applied. Without the bypass the
  step waits on the migrations it is itself applying.

`web` declares no dependency on `celery`. It waits on the migrations themselves,
in its entrypoint, so a sick worker delays nothing once they are applied. That
gate is also the net for the one case `pre_start` does not cover: Compose skips a
step when the container restarts under its restart policy, which is what a host
reboot does.

Composer's only remaining part in this chain is supplying `DLUX_MIGRATOR_FLAGS`,
which Compose interpolates into the migrator step — that is how a deploy's
`-mm` / `-nm` / `-a` reach a hook that is otherwise static in the file.
`composer check` FAILs a host whose Compose predates 5.3.0, because an older
plugin ignores `pre_start` rather than rejecting it.

## Composer is required for updates (changed in 1.8.0)

**The inline updater requires Composer running as a service in the deployment,
on its latest stable image — in addition to Composer being the deployer.** This
is a hard requirement from DjangoLux 1.8.0, not a recommendation. A DjangoLux
stack without a Composer service has no update path: it can neither learn what
is available nor apply it.

DjangoLux is deliberately being stripped of outbound responsibilities. Fetching,
verifying, staging, activating, health-gating and rolling back all happen from
outside the container, where they can be done safely. What remains inside is
intent and state.

Projects generated by the 1.8.0 scaffold already ship the Composer services.
For a project generated earlier, `composer check` FAILs and `composer check
--fix` installs them:

```
composer check          # reports what is missing
composer check --fix    # installs docker-socket-proxy, composer-executor, composer-agent
docker compose up -d docker-socket-proxy composer-executor composer-agent
composer dlux-update --dry-run   # proves the path end to end
```

The install derives the image and labels from the project's own `web` service,
adds the `composer_agent_state` and `composer_exec_sock` volumes and the
`docker_proxy` network, validates with `docker compose config`, and preserves
the original under `.xpose/` before writing. It is idempotent and refuses
anything it does not recognize rather than guessing.

`DLUX_UPDATE_EXECUTOR="inline"` restores the legacy in-container path for a
deployment whose Composer predates the `dlux.package_update` action. It is a
migration aid with a deadline, not a supported way to run without Composer: both
it and the in-container executor are removed in 1.9.0.

## Who executes an update (changed in 1.8.0)

Since DjangoLux 1.8.0, **Composer performs inline updates**; DjangoLux only
states intent. An approved apply, rollback or check writes a file on the shared
`dlux_runtime` volume, and Composer's update loop stages the release, verifies
its PyPI Trusted Publisher attestation, activates it and health-gates the
restart **from outside the container being swapped** — so it can roll back a
release that stopped that container from starting, which an in-container updater
cannot do for itself.

| File on `state/` | Written by | Meaning |
| --- | --- | --- |
| `package-update-request.json` | DjangoLux | apply or roll back this version |
| `package-update-request.json.ack` | Composer | that request finished (with its exit code) |
| `package-available.json` | Composer | what release is installable, and whether it is `inline_safe` |

DjangoLux makes no network requests of its own on this path. If Composer has not
published `package-available.json` yet, the panel shows the check as *unknown* —
never as "up to date".

**What did not change.** `inline_safe` still decides whether a release can be
applied to the runtime volume or needs a project image rebuild; it is read from
the manifest inside the wheel, and Composer honours it rather than second-
guessing it. Rollback did not go away — it moved to the side that can perform it
safely.

**The `dlux-updater` service stays.** It is not only the update executor: it runs
`dlux_reconcile`, applies migrations, and `web` gates its own start on its
health. It also drains the update queue — that is what writes the intent file.
DjangoLux 1.9.0 removes the executor code *inside* it, not the service.

**Migrating a deployment that already runs Composer.** Nothing in `compose.yml`
has to change: the trigger path is derived, and every Composer update loop
already mounts the volume. `composer check` confirms readiness — it verifies a
loop exists and mounts `dlux_runtime` — and `composer check --fix` is safe to
run before or after the 1.8.0 upgrade.

`dlux enable-updater` is deprecated for the same reason and removed in 1.9.0.
