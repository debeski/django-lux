...
The `control_url` shown on the Control Panel is read from `agent-status.json` (never hardcoded), so it persists after a disconnect and reflects the last enrollment. When a previously-connected deployment is no longer enrolled — or the control plane sets `revoked` — the panel renders a distinct **Disconnected** state and warning banner, and the update worker posts a one-time warning notification to superadmins (deduped on the runtime volume, re-armed on reconnect) so the drop is noticed even without opening the panel.
