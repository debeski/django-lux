...
- **Theme Picker Location** (`theme_picker_location`): where users switch theme.
  - `sidebar_toolbar` (default) — in the sidebar toolbar, next to density and reorder.
  - `titlebar` — a titlebar action beside the language switcher, cycling through the allowed themes.
  - `disabled` — nowhere in the chrome; the Options page only.

  The sidebar toolbar can only host the picker while it exists, so turning the
  sidebar or its toolbar off makes that option unchoosable and moves the choice to
  `titlebar`. Stored values resolve the same way at runtime, so a picker is never
  lost by disabling the sidebar.

  The Options **Themes** card disappears when it would be redundant: exactly two
  themes allowed, user theme override on, and the sidebar toolbar already showing
  its direct two-theme toggle. Any other combination keeps the card.
...
**Creating and managing backups.** The page uses the same Dlux form-density fields, themed glass surfaces, `DluxFileInput` file card, and `dlux-table-shell` tables as the rest of the system UI. It builds backups in the background through Celery (`dlux.tasks.build_system_backup`) with polling, or inline when no worker is available. Pending/running rows initially have zero byte/row/file counts because the artifact does not exist yet; their model-by-model progress percentage and current stage are persisted and rendered in the history table. While the page remains open, a superuser-only no-cache feed discovers manual, scheduled, and updater-triggered runs and replaces those provisional values with the completed metrics plus download/restore controls without a manual reload. An in-progress `.dlb` may already be growing in storage, but it is deliberately excluded from the external-file list until its `SystemBackup` row completes. The optional passphrase is passed only to the active inline run or Celery task; it is not stored on the `SystemBackup` row. Completed backups can be downloaded, deleted, or restored. `.dlb` files can also be uploaded or copied directly into the protected backup folder (`dlux_backups/` in Django default storage by default — keep the reverse-proxy guard from the reports-backup section); the page lists such external files and can restore from them, which is the path for disaster recovery onto a rebuilt server. **Copying is the route for anything large.** The browser upload passes through the reverse proxy, whose body limit is `CADDY_MAX_SIZE` / `NGINX_MAX_SIZE` (`10M` in the generated `.secrets/.env`), and then through the app's own `DLUX_DLB_UPLOAD_MAX_MB` cap (512 MB) and the gunicorn request timeout — a multi-GB snapshot clears none of the three. When the proxy truncates the body, Django receives no complete file part and the page reports that the upload was cut off rather than blaming the file; the `.dlb` itself is fine and still opens in the viewer. Copy such files onto the server and restore them from the external-file list. Backup history identifies whether each run was manual, scheduled, or created as an inline-update prerequisite.
...
**Watching a restore.** A restore reports live progress the same way a backup does, on its own row in the Restore History table: a progress bar, the current phase, and how many seconds ago it last reported. The phases are the ones that actually take time — reading and **decrypting** the container (reported against the file's byte size, and the longest phase by far on a multi-GB `.dlb`), the per-model database **load**, the per-file **media** writes, and **finalizing** (cache and session invalidation). The table polls `/sys/backup/restore/status/` while a run is active and reloads the page once it finishes, because a completed restore has replaced the data the rest of the page was rendered from.

Two details are consequences of what a restore *is*, and are deliberate. First, the database phase runs inside a single transaction, so a progress row written from inside it would stay invisible to the polling web process until the entire load commits — precisely the stretch an operator most wants to watch. Every tick is therefore mirrored into the cache and overlaid on the persisted row at read time; the row remains the source of truth, so progress still survives a page reload, a Celery handoff, and a cache that is not shared between processes. Second, there is no "restore started" drawer notification to match the backup's. A restore wipes and reloads every table it owns, `DluxNotification` included, so an item created before the load would be deleted by the restore itself; only the terminal `restore_completed` / `restore_failed` notice, written after the transaction commits, can survive.
...
Open **Options → Admin panel → Asset Manager** to manage reusable images and WOFF2 fonts in the standard Dlux dynamic modal. Assets already referenced by branding or a registered font cannot be deleted. Use the file card's clear action in System Settings to remove a field reference without deleting the reusable file. See [Managed assets](managed-assets.md) for formats, limits, and storage behavior.
