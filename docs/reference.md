...
| `/sys/api/dlux-ops/state/` | `GET` | Superuser-only deployment-row state: the available operations, the newest run, the last deployment check, and the resident Composer's version against its channel |
