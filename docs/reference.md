...
| `/sys/api/dlux-ops/run/` | `POST` | Superuser-only CSRF-protected, audited request for one named deployment operation (`operation`: `check`, `check-fix-apply`, `agent-update`); the write operations also re-verify `current_password`, and `check-fix-apply` may only apply what the last check showed |
