"""
Resolver-driven route discovery and sidebar rendering helpers.

Discovery is global: :func:`discover_routes` walks the URLconf once and returns
EVERY named route, classified but unfiltered. Features do not share a filter —
each one reads the same catalog through a profile (`sidebar`, `navbar`,
`navbar_root`, `search`, `landing`) declared in :mod:`dlux.system.constants`.
That split is what lets an ``add`` page be a Nav Bar child and a search result
while still staying out of the sidebar's default entries.

This module powers:
1. Global discovery of named routes and their per-feature projections.
