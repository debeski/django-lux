document.addEventListener('DOMContentLoaded', function() {
    document.addEventListener('click', function(e) {
        const printButton = e.target.closest('[data-dl-user-report-print]');
    ...
    document.addEventListener('shown.bs.tab', function(e) {
        const tab = e.target.closest('[data-dl-user-report-window]');
        if (!tab) return;
        const report = tab.closest('[data-dl-user-report]');
        // The export button may have been pinned into the modal footer (moved out of
        // [data-dl-user-report]); fall back to a document-wide lookup. Only one report
        // modal is open at a time, so this is unambiguous.
        const exportLink = report?.querySelector('[data-dl-user-report-export-base]')
            || document.querySelector('[data-dl-user-report-export-base]');
    ...
    function updateUserReportActivityPager(report, requestedPage) {
        const timeline = report.querySelector('[data-dl-user-report-activity]');
        const pager = report.querySelector('[data-dl-user-report-activity-pagination]');
        if (!timeline || !pager) {
            return;
        }

        const items = Array.from(timeline.querySelectorAll('[data-dl-user-report-activity-item]'));
        ...
        const state = pager.querySelector('[data-dl-user-report-page-state]');
        if (state) {
        ...
        const previous = pager.querySelector('[data-dl-user-report-page-prev]');
        const next = pager.querySelector('[data-dl-user-report-page-next]');
        if (previous) previous.disabled = currentPage <= 1;
        if (next) next.disabled = currentPage >= totalPages;
    }

    function initUserReportActivityPagers(root) {
        root.querySelectorAll('[data-dl-user-report]').forEach(report => {
    ...
    document.addEventListener('click', function(e) {
        const previous = e.target.closest('[data-dl-user-report-page-prev]');
        const next = e.target.closest('[data-dl-user-report-page-next]');
        if (!previous && !next) {
            return;
        }

        const report = e.target.closest('[data-dl-user-report]');
        const timeline = report?.querySelector('[data-dl-user-report-activity]');
        if (!report || !timeline) {
            return;
        }
