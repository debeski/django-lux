document.addEventListener('DOMContentLoaded', function () {
    ...
    const loadingHandles = new WeakMap();

    // Shared spinner/disable/restore behaviour. The primitive restores the
    // button's markup and disabled state as they were when loading started, so
    // callers must re-apply anything the cooldown changed mid-flight.
    function setButtonLoading(button, loading) {
        ...
        if (loading) {
            loadingHandles.set(button, window.DluxLoadingButton.start(button));
            return;
        }
        const handle = loadingHandles.get(button);
        if (handle) {
            handle.done();
            loadingHandles.delete(button);
        } else {
    ...
    function startCooldown(seconds) {
        ...
        renderCooldown(cooldownRemaining);
        ...
        emailBtn.addEventListener('click', function () {
            ...
            fetch(emailBtn.dataset.url, {
                ...
                .finally(function () {
                    ...
                    // The restore reinstated the pre-flight label and enabled
                    // state; a cooldown started during the request would only
                    // reappear on its next tick, leaving a clickable gap.
                    renderCooldown(cooldownRemaining);
                });
        });
    }
