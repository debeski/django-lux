...
def manage_scopes(request):
    ...
    context = {'table': table}
    html = render_to_string('dlux/scopes/scope_manager.html', context, request=request)
...
def save_scope(request, pk=None):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            table = ScopeTable(Scope.objects.all(), request=request)
            RequestConfig(request).configure(table)
            html = render_to_string('dlux/scopes/scope_manager.html', {'table': table}, request=request)
