...
def options_view(request):
    ...
    if show_system_diagnostics:
        ...
        # The Composer that deployed this stack. `COMPOSER_VERSION` is injected
        # into every service by the Composer run that created it, so it is gone
        # from web the moment anything else recreates that container — which is
        # why this row kept vanishing. The agent publishes the same value, and
        # the agent IS recreated by a deploy, so its copy outlives web's.
        #
        # The agent's own version is a different fact and lives on the
        # deployment rows in Updates and maintenance, not here.
        ...
        if not composer_version:
            try:
                ...
                composer_version = str(_agent_status.get('deployer_version') or '').strip()
            except Exception:
                composer_version = ''
