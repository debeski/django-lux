...
class ScaffoldTests(unittest.TestCase):
    def test_startproject_creates_expected_files(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            self.assertTrue((target / ".nginx" / "default.conf.template").exists())
            self.assertTrue((target / ".nginx" / "maintenance.html").exists())
            ...
            nginx_contents = (target / ".nginx" / "default.conf.template").read_text(encoding="utf-8")
            ...
            self.assertEqual(len([line for line in env_contents.splitlines() if line.strip()]), 15)
            ...
            self.assertIn("./.nginx/default.conf.template:/etc/nginx/templates/default.conf.template:ro", compose_contents)
            self.assertIn("NGINX_ENVSUBST_FILTER:", compose_contents)
            self.assertIn("NGINX_SERVER_NAME: ${NGINX_SERVER_NAME:-localhost}", compose_contents)
            self.assertIn("NGINX_MAX_SIZE: ${NGINX_MAX_SIZE:-10M}", compose_contents)
            ...
            self.assertNotIn("/var/run/docker.sock", compose_contents)
