import os
import sys
from pathlib import Path


REPO_ROOT = Path(__file__).resolve().parents[2]
sys.path.insert(0, str(REPO_ROOT))

TEST_LABELS = [
    'dlux.tests.test_accent_edges',
    'dlux.tests.test_access',
    'dlux.tests.test_activitylog',
    'dlux.tests.test_admin_actions',
    'dlux.tests.test_agent_bridge',
    'dlux.tests.test_api',
    'dlux.tests.test_arabic_search',
    'dlux.tests.test_asset_fields',
    'dlux.tests.test_assisted_entry',
    'dlux.tests.test_audit_visibility',
    'dlux.tests.test_auth_security',
    'dlux.tests.test_backup_recovery',
    'dlux.tests.test_card_edges',
    'dlux.tests.test_celery_health',
    'dlux.tests.test_channels',
    'dlux.tests.test_check_policy',
    'dlux.tests.test_ops',
    'dlux.tests.test_session_health',
    'dlux.tests.test_context_processors',
    'dlux.tests.test_control_link',
    'dlux.tests.test_custom_fonts',
    'dlux.tests.test_custom_languages',
    'dlux.tests.test_custom_themes',
    'dlux.tests.test_data_reset',
    'dlux.tests.test_defaults_and_urls',
    'dlux.tests.test_dependent_settings',
    'dlux.tests.test_deployment_config_docs',
    'dlux.tests.test_dialog_reset',
    'dlux.tests.test_dlux_setup',
    'dlux.tests.test_doctor',
    'dlux.tests.test_downstream_compat',
    'dlux.tests.test_email_step',
    'dlux.tests.test_groups',
    'dlux.tests.test_import_graph',
    'dlux.tests.test_inspector_shell',
    'dlux.tests.test_logging_search_ui',
    'dlux.tests.test_lookup',
    'dlux.tests.test_lookup_js',
    'dlux.tests.test_m2m',
    'dlux.tests.test_managed_assets',
    'dlux.tests.test_manifest_schema_v2',
    'dlux.tests.test_middleware',
    'dlux.tests.test_migrator',
    'dlux.tests.test_modal_content_init',
    'dlux.tests.test_models',
    'dlux.tests.test_notifications',
    'dlux.tests.test_options_layout',
    'dlux.tests.test_options_registry',
    'dlux.tests.test_package_facades',
    'dlux.tests.test_package_handoff',
    'dlux.tests.test_password_reset',
    'dlux.tests.test_permissions_ui',
    'dlux.tests.test_prune_assets',
    'dlux.tests.test_registration',
    'dlux.tests.test_render_cost',
    'dlux.tests.test_report_backup',
    'dlux.tests.test_ribbon',
    'dlux.tests.test_ribbon_tabs',
    'dlux.tests.test_row_actions_style',
    'dlux.tests.test_runtime_contract',
    'dlux.tests.test_runtime_supervisor',
    'dlux.tests.test_scaffold',
    'dlux.tests.test_scanlink_gate',
    'dlux.tests.test_scanlink_releases',
    'dlux.tests.test_scope_default_theme',
    'dlux.tests.test_search',
    'dlux.tests.test_seed',
    'dlux.tests.test_settings_diff',
    'dlux.tests.test_settings_import_views',
    'dlux.tests.test_settings_step_anchors',
    'dlux.tests.test_settings_theme_persistence',
    'dlux.tests.test_setup_js_modules',
    'dlux.tests.test_sidebar_discovery',
    'dlux.tests.test_sidebar_toggle_icon',
    'dlux.tests.test_signals',
    'dlux.tests.test_smtp_relay',
    'dlux.tests.test_smtp_timeouts',
    'dlux.tests.test_suite_registration',
    'dlux.tests.test_system_backup',
    'dlux.tests.test_system_settings_previews',
    'dlux.tests.test_system_registry',
    'dlux.tests.test_table_edges',
    'dlux.tests.test_table_translation',
    'dlux.tests.test_tables',
    'dlux.tests.test_template_comment_finder',
    'dlux.tests.test_theme_picker_location',
    'dlux.tests.test_theme_toggle',
    'dlux.tests.test_theme_tokens',
    'dlux.tests.test_theme_variables',
    'dlux.tests.test_titlebar_action_rail',
    'dlux.tests.test_transaction_safety',
    'dlux.tests.test_translation_coverage',
    'dlux.tests.test_unsaved_guard',
    'dlux.tests.test_updater',
    'dlux.tests.test_utils',
    'dlux.tests.test_utils_discovery',
    'dlux.tests.test_views',
]


def run_all_tests(argv=None):
    """Run the package CI suite through Django's test runner."""
    os.environ.setdefault('DJANGO_SETTINGS_MODULE', 'dlux.tests.settings')

    from django.core.management import execute_from_command_line

    args = list(argv or sys.argv[1:])
    verbosity = []
    if not any(arg.startswith('--verbosity') or arg == '-v' for arg in args):
        verbosity = ['--verbosity=2']
    execute_from_command_line(
        [sys.argv[0], 'test', *TEST_LABELS, *verbosity, *args]
    )
    return 0


if __name__ == '__main__':
    sys.exit(run_all_tests())
