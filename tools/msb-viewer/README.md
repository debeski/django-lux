# Microsys Backup Viewer (`.msb`)

A standalone, **zero-dependency** desktop viewer for the encrypted system
backups (`.msb`) produced by [django-microsys](../../). It decrypts a backup
...
The viewer must read the exact `.msb` container that `microsys/backup.py`
writes: a cleartext `MSB1` header followed by Fernet-encrypted ZIP chunks
...
[`msb.go`](msb.go). The result is a **single static binary with no third-party
...
```sh
cd tools/msb-viewer
go build -o msb-viewer .
```

Cross-compile for other platforms (no CGo, so this just works):

```sh
GOOS=windows GOARCH=amd64 go build -o msb-viewer.exe .
GOOS=darwin  GOARCH=arm64 go build -o msb-viewer-macos .
GOOS=linux   GOARCH=amd64 go build -o msb-viewer-linux .
```

## Run

```sh
./msb-viewer                       # opens the UI; pick a file in the browser
./msb-viewer path/to/system-backup.msb
```
...
Every `.msb` header records, in cleartext, how it was encrypted. The viewer
...
- **Overview** — counts, generation time, Microsys version, encryption params,
  superuser policy.
