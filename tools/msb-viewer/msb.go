// msb.go — parsing and decryption of Microsys backup (.msb) containers.
//
// Container layout (must match microsys/backup.py):
//
//	"MSB1"                4-byte magic
...
// Encryption mirrors the "encryption" block written by write_msb_container.
...
// Metadata is the cleartext header of an .msb file.
...
func parseHeader(r io.Reader) (*Metadata, error) {
	magic := make([]byte, len(msbMagic))
	if _, err := io.ReadFull(r, magic); err != nil {
		return nil, errors.New("not a Microsys backup (.msb) file")
	}
	if string(magic) != msbMagic {
		return nil, errors.New("not a Microsys backup (.msb) file")
	}
	var lenBuf [4]byte
	...
	if meta.Kind != "microsys-system-backup" {
