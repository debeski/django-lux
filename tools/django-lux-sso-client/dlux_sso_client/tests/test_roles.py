...
from dlux_sso_client.roles import apply_role_mapping, extract_role, is_role_allowed


class RoleMappingTests(TestCase):
    def test_extracts_nested_dlux_role_claim(self):
        self.assertEqual(extract_role({"dlux_sso": {"role": "Admin"}}), "admin")

    def test_extracts_flat_dlux_role_claim(self):
        self.assertEqual(extract_role({"dlux_sso_role": "Staff"}), "staff")
