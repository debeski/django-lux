...
def get_client_config():
    config = getattr(settings, "DLUX_SSO_CLIENT", {})
    return config if isinstance(config, dict) else {}


def extract_role(claims):
    if claims.get("dlux_sso_role"):
        return str(claims["dlux_sso_role"]).strip().lower()
    dlux_claim = claims.get("dlux_sso") or {}
    if isinstance(dlux_claim, dict) and dlux_claim.get("role"):
        return str(dlux_claim["role"]).strip().lower()
    return str(claims.get("dlux:role") or claims.get("role") or "").strip().lower()
