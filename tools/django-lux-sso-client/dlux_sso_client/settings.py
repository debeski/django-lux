def _coerce_list(scope, key):
    ...
    if not isinstance(value, list):
        raise TypeError(f"{key} must be a list or tuple before configure_dlux_sso() runs")
    return value
...
def configure_dlux_sso(
    ...
    *,
    ...
    """
    Configure a Django project as an OIDC client of a Dlux SSO provider.

    The helper intentionally does not import or require django-lux.
    """
    if not isinstance(scope, dict):
        raise TypeError("configure_dlux_sso() expects the result of globals() from settings.py")
    ...
    for app_label in reversed(["dlux_sso_client", "mozilla_django_oidc"]):
        if app_label in installed_apps:
            installed_apps.remove(app_label)
        installed_apps.insert(0, app_label)

    backend_path = "dlux_sso_client.backends.DluxSSOAuthenticationBackend"
    ...
    client_config = dict(scope.get("DLUX_SSO_CLIENT", {}))
    ...
    scope["DLUX_SSO_CLIENT"] = client_config
