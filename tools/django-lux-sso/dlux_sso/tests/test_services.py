...
from dlux_sso.services import build_userinfo_claims, validate_redirect_uri
...
class RedirectPolicyTests(TestCase):
    ...
    def test_userinfo_claims_are_portable_and_do_not_export_permissions(self):
        ...
        app = SimpleNamespace(client_id="client-123", dlux_sso_policy=policy)

        claims = build_userinfo_claims(user, app)

        self.assertEqual(claims["sub"], "42")
        self.assertEqual(claims["dlux_sso_role"], "user")
        self.assertEqual(claims["dlux_sso"]["role"], "user")
        self.assertEqual(claims["dlux_sso_client_id"], "client-123")
