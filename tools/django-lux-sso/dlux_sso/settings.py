...
def _coerce_list(scope, key):
    ...
    if not isinstance(value, list):
        raise TypeError(f"{key} must be a list or tuple before dlux_sso_settings() runs")
    return value
...
def dlux_sso_settings(scope):
    """
    Apply optional Dlux SSO provider settings to a Django settings module.

    The core Dlux package intentionally does not call this helper. Projects
    that want to act as an SSO provider opt in from settings.py.
    """
    if not isinstance(scope, dict):
        raise TypeError("dlux_sso_settings() expects the result of globals() from settings.py")

    installed_apps = _coerce_list(scope, "INSTALLED_APPS")
    middleware = _coerce_list(scope, "MIDDLEWARE")

    for app_label in reversed(["dlux_sso", "oauth2_provider"]):
    ...
    if not scope.get("OIDC_RSA_PRIVATE_KEY"):
        env_key = scope.get("DLUX_SSO_PRIVATE_KEY")
        key_path = scope.get("DLUX_SSO_PRIVATE_KEY_PATH")
        if env_key:
            scope["OIDC_RSA_PRIVATE_KEY"] = env_key
        elif key_path:
            scope["OIDC_RSA_PRIVATE_KEY"] = Path(key_path).read_text(encoding="utf-8")

    provider_config = dict(scope.get("DLUX_SSO_PROVIDER", {}))
    provider_config.setdefault("issuer", scope.get("DLUX_SSO_ISSUER", ""))
    ...
    scope["DLUX_SSO_PROVIDER"] = provider_config
